...
export default function ItemDetailPage() {
  ...
  const recommendationsQuery = useQuery({
    queryKey: ['item-recommendations', id],
    queryFn: () => itemsService.getRecommendedAccessories(id!),
    enabled: !!id,
  });
      ...
      <ShopLayout>
        <div className="py-12">
          <ErrorState
            ...
            description="The requested item does not exist or is currently unavailable."
            onRetry={refetch}
          />
        </div>
      </ShopLayout>
  ...
  const { name, price, image, description, category, available } = item;
          ...
          <div className="flex flex-col justify-between py-2 gap-6">
            ...
            {available ? (
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100 mt-auto">
                ...
                <div className="flex gap-3 pt-2">
                  <a
                    href={`https://wa.me/971506767915?text=${encodeURIComponent(`Hi Al Nader Pets! 🐾 I am inquiring about the item "${name}" which is currently marked as SOLD.`)}`}
                    target="_blank"
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 flex-1 min-w-0">
                {visibleRecommendations.map((rec: any) => (
                  <ItemCard key={rec.id} item={rec} />
                ))}
              </div>
