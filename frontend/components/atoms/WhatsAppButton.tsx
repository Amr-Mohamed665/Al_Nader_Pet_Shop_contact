...
/**
 * Builds a WhatsApp wa.me URL with a smart pre-filled message.
 * If the customer has items in their cart, the message will list them
 * and express interest in knowing more.
 */
export function buildWhatsAppUrl(cartItems: { name: string; quantity: number }[]): string {
  let message: string;

  if (cartItems.length === 0) {
    message = "Hi Al Nader Pet Shop! 🐾 I'd like to know more about your pets and accessories. Can you help me?";
...
export default function WhatsAppButton() {
  ...
  return (
    <a
      ...
      aria-label={
        hasItems
          ? `Chat on WhatsApp about your ${items.length} cart item${items.length > 1 ? 's' : ''}`
    ...
    >
      ...
      <span className="whatsapp-float-label" aria-hidden="true">
        {hasItems ? `Ask about your cart (${items.length})` : 'Chat with us'}
      </span>
