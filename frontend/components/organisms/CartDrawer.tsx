...
import { buildWhatsAppUrl } from '@/components/atoms/WhatsAppFloat';
...
export default function CartDrawer() {
  ...
  // Build smart WhatsApp pre-filled message from current cart items
      ...
      >
        ...
        {items.length > 0 && (
          <div className="border-t border-slate-100 px-6 py-6 bg-slate-50/50 space-y-4">
            ...
            {/* WhatsApp enquiry — auto-fills message with cart items */}
            <a
              ...
              onClick={closeCart}
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-[#25D366] hover:bg-[#1ebe5d] text-white text-xs font-bold transition-all duration-200 shadow-sm hover:shadow-md hover:shadow-green-400/30 hover:scale-[1.02] active:scale-[0.98]"
            >
              <i className="fa-brands fa-whatsapp text-[16px]" aria-hidden="true" />
              <span>Ask about {items.length === 1 ? 'this item' : 'these items'} on WhatsApp</span>
            </a>
          </div>
        )}
      </div>
